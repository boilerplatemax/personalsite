...
export default function SearchMenu({ items }) {
  const { searchHandler, filterHandler, listHandler, randomSearchHandler } =
      ...
      <div>
        <div className="search-menu">
          <CloseButton
            ...
            className="float-end"
          />
          <div className="search-menu__title">
            <h4>Search Filters</h4>
          </div>
          <h2>
            {items ? items.length : 0}{" "}
            {items && items.length > 1 ? "Results" : "Result"}
          </h2>
          <div className="search-menu__body">
            ...
            <button className="btn btn--primary" onClick={randomSearchHandler}>
              Surprise Me
            </button>
          </div>
        </div>
